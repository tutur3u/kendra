...
export function HomeStudioClient({ content }: { content: KendraContent }) {
	const { availability, studioSpecs } = content;
		...
		<main className="bg-white">
			...
			>
				...
				>
					...
					<p className={cn(bodyText, "max-w-[280px]")}>
						Clean remote sessions from Alberta, Canada.
					</p>
				</motion.div>
			...
			>
				...
				>
					{availability.map((item) => (
						<li
							key={item}
							className="text-[clamp(1.1rem,1.8vw,1.4rem)] font-light tracking-tight text-ink-soft border-b border-line pb-6 last:border-0"
						>
							- {item}
						</li>
					))}
