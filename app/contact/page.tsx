...
import { availability, contactIntro, site } from "../content";
		...
		<main className="bg-white">
			...
			>
				<motion.div variants={fadeInUp} initial="initial" animate="animate">
					<a className="text-[1.2rem] font-bold italic tracking-tight underline decoration-accent/30 underline-offset-8 transition-all hover:text-accent" href={`mailto:${site.email}`}>
						{site.email}
					</a>
				</motion.div>
			</PageHero>
				...
				>
					...
					<ul className="flex flex-col gap-4 border-t border-line pt-10">
						{availability.map((item) => (
							<li key={item} className="text-sm font-medium tracking-tight text-ink-soft">
								— {item}
							</li>
						))}
					</ul>
