        ...
        ? rawItems
            ...
            .filter(
              (item) =>
                ...
                Boolean(
                  item.project || item.role || Object.keys(item.visual).length,
                ),
            )
        : [];
...
export function normalizeKendraEditableSiteContent(
  ...
  return {
    ...
    experienceGroups: normalizeExperienceGroups(
      record.experienceGroups,
      options,
    ),
...
export function validateKendraEditableSiteContent(
  ...
  content.studioSpecs.forEach((item, index) => {
    addRequiredError(
      errors,
      `studioSpecs.${index}.label`,
      item.label,
      "Spec label",
    );
    addRequiredError(
      errors,
      `studioSpecs.${index}.value`,
      item.value,
      "Spec value",
    );
  });
  ...
  content.experienceGroups.forEach((group, groupIndex) => {
    ...
    group.items.forEach((item, itemIndex) => {
      ...
      if (
        typeof item.visual?.image === "string" &&
        item.visual.image &&
        !isValidImageReference(item.visual.image)
      ) {
        errors[`${base}.visual.image`] = "Use a public path or full image URL.";
      }
    });
  });
...
export function readKendraDeliverySiteContent(
  ...
  const entry = collection?.entries.find(
    (item) =>
      item.slug === KENDRA_SITE_CONTENT_ENTRY_SLUG &&
      item.status === "published",
  );

  if (!entry) return null;

  const content = normalizeKendraEditableSiteContent(
    entry.profile_data.content,
  );
  const errors = validateKendraEditableSiteContent(content);
